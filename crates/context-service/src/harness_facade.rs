...
/// Maximum number of entries retained in each write-driven facade cache.
///
/// The owner remains the source of truth.  Once this bounded local index evicts an entry, the
/// corresponding reference is revalidated through the owner before it can be forwarded again.
pub const MAX_FACADE_CACHE_ENTRIES: usize = crate::control::MAX_ITEMS;
/// Maximum number of item references retained by the local validation index.
///
/// A draft can contain all selected items plus the bounded note set, so this is intentionally
/// larger than the per-response item bound while remaining finite.
pub const MAX_FACADE_ITEM_CACHE_ENTRIES: usize =
    crate::control::MAX_ITEMS + crate::control::MAX_NOTES;
...
pub trait HarnessOwnerPort {
    ...
    /// Recover a receipt using the caller's original idempotency key.
    ///
    /// This is deliberately separate from [`Self::get_receipt`]: the command id is generated by
    /// the owner and may never have reached the caller when a reply is lost.  Implementations
    /// that do not expose this owner-side lookup fail closed with `Unsupported`.
    fn get_receipt_by_idempotency_key(
        ...
        self.get_receipt_by_reference(auth, scope, idempotency_key)
    }
    /// Compatibility alias for owners that call the caller-known value a reference key.
    fn get_receipt_by_reference(
        &mut self,
        _auth: &ProtectedAuthReference,
        _scope: &Scope,
        _reference_key: &str,
...
/// Bounded local reference-cache occupancy for diagnostics and regression tests.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct FacadeCacheStatus {
    pub item_refs: usize,
    pub revisions: usize,
    pub drafts: usize,
    pub previews: usize,
...
struct ReferenceIndex {
    ...
    item_order: VecDeque<(String, u64)>,
    revision_ids: BTreeSet<String>,
    revision_id_order: VecDeque<String>,
    revision_order: VecDeque<String>,
    revisions: BTreeMap<String, Revision>,
    draft_ids: BTreeSet<String>,
    draft_id_order: VecDeque<String>,
    draft_order: VecDeque<String>,
...
impl ReferenceIndex {
    ...
    fn cache_item(&mut self, item: ItemRef) -> bool {
        bounded_map_insert(
            &mut self.item_refs,
            &mut self.item_order,
            (item.item_id.clone(), item.version),
            item,
            MAX_FACADE_ITEM_CACHE_ENTRIES,
        )
    }

    fn cache_revision(&mut self, revision: Revision) -> bool {
        let revision_id = revision.revision_id.clone();
        let evicted_set = bounded_set_insert(
            &mut self.revision_ids,
            &mut self.revision_id_order,
            revision_id.clone(),
            MAX_FACADE_CACHE_ENTRIES,
        );
        let evicted_map = bounded_map_insert(
            &mut self.revisions,
            &mut self.revision_order,
            revision_id,
            revision,
            MAX_FACADE_CACHE_ENTRIES,
        );
        evicted_set || evicted_map
    }

    fn cache_draft(&mut self, draft: Draft) -> bool {
        let draft_id = draft.draft_id.clone();
        let evicted_set = bounded_set_insert(
            &mut self.draft_ids,
            &mut self.draft_id_order,
            draft_id.clone(),
            MAX_FACADE_CACHE_ENTRIES,
        );
        let evicted_map = bounded_map_insert(
    ...
    fn cache_preview(&mut self, preview: &Preview) -> bool {
        bounded_set_insert(
            &mut self.preview_ids,
    ...
    fn cache_receipt(&mut self, receipt: &Receipt) -> bool {
        bounded_set_insert(
            &mut self.receipt_ids,
            &mut self.receipt_order,
...
fn bounded_set_insert(
    ...
    if set.contains(&value) {
        order.retain(|current| current != &value);
    }
...
impl<O: HarnessOwnerPort> HarnessBackedContextService<O> {
    ...
    pub fn cache_status(&self) -> FacadeCacheStatus {
        FacadeCacheStatus {
            item_refs: self.index.item_refs.len(),
            revisions: self.index.revisions.len(),
            drafts: self.index.drafts.len(),
            previews: self.index.preview_ids.len(),
            receipts: self.index.receipt_ids.len(),
        }
    }
    ...
    pub fn handle_http_at(&mut self, request: &HttpRequest, now: u64) -> HttpResponse {
        ...
        let result = match (request.method.as_str(), tail) {
            ...
            ("GET", ["commands"]) if query.is_empty() => {
                self.receipts(&facade_request).and_then(to_value)
            }
            ("GET", ["commands"]) => self
                .http_receipt_recovery(&facade_request, &query)
                .and_then(to_value),
            ...
            ("GET", ["commands", command_id]) => self
                ...
                .and_then(to_value),
            ("GET", ["commands", "by-idempotency", idempotency_key]) if query.is_empty() => self
                .recover_receipt(&facade_request, idempotency_key)
                .and_then(to_value),
            ("GET", ["commands", "by-idempotency-key", idempotency_key]) if query.is_empty() => {
                self.recover_receipt(&facade_request, idempotency_key)
                    .and_then(to_value)
            }
            ("GET", ["commands", "by-reference", reference_key]) if query.is_empty() => self
                .recover_receipt(&facade_request, reference_key)
                .and_then(to_value),
            ("POST", ["drafts"]) => self
    ...
    fn http_receipt_recovery(
        &mut self,
        request: &FacadeRequest,
        query: &[(String, String)],
    ) -> FacadeResult<Receipt> {
        if query.len() != 1 {
            return Err(FacadeError::invalid("invalid_query"));
        }
        let (key, value) = &query[0];
        if !matches!(
            key.as_str(),
            "idempotency_key" | "reference_key" | "reference"
        ) {
            return Err(FacadeError::invalid("invalid_query"));
        }
        self.recover_receipt(request, value)
    }
    ...
    pub fn eligible_items(
        ...
        let scope = self.config.scope.clone();
        let draft_references = self.cached_draft_references();
        let items = self
            .client
        ...
        let items = match self.project_items(items, include_content) {
            Ok(items) => items,
            Err(error) => {
                self.index.clear_items();
                return Err(error);
            }
        };
        self.index.item_refs.clear();
        self.index.item_order.clear();
        let mut evicted = false;
        for item in &items {
            evicted |= self.index.cache_item(item.item.clone());
        }
        for item in draft_references {
            evicted |= self.index.cache_item(item);
        }
        self.index.items_complete = !evicted;
        Ok(items)
    }
    ...
    pub fn revisions(&mut self, request: &FacadeRequest) -> FacadeResult<Vec<Revision>> {
        ...
        self.index.clear_revisions();
        let mut evicted = false;
        for revision in revisions.iter().cloned() {
    ...
    pub fn drafts(&mut self, request: &FacadeRequest) -> FacadeResult<Vec<Draft>> {
        ...
        for draft in drafts.iter().cloned() {
            ...
            evicted |= self.index.cache_draft(draft);
        }
        self.index.drafts_complete = !evicted;
        Ok(drafts)
    }

    pub fn read_draft(&mut self, request: &FacadeRequest, draft_id: &str) -> FacadeResult<Draft> {
        self.authorize(request, FacadePermission::MetadataRead, false)?;
        self.ensure_known_id(draft_id, ReferenceKind::Draft)?;
        self.require_owner("drafts")?;
        let scope = self.config.scope.clone();
    ...
    pub fn previews(&mut self, request: &FacadeRequest) -> FacadeResult<Vec<Preview>> {
        ...
        for preview in &previews {
            evicted |= self.index.cache_preview(preview);
        }
        self.index.previews_complete = !evicted;
        Ok(previews)
    }
    ...
    pub fn read_preview(
        ...
        self.authorize(request, FacadePermission::MetadataRead, false)?;
        self.ensure_known_id(preview_id, ReferenceKind::Preview)?;
        self.require_owner("previews")?;
        let scope = self.config.scope.clone();
    ...
    pub fn read_receipt(
        ...
        self.authorize(request, FacadePermission::MetadataRead, false)?;
        self.ensure_known_id(command_id, ReferenceKind::Receipt)?;
    ...
    /// Recover an owner receipt using the caller's original idempotency/reference key.
    ///
    /// Unlike [`Self::read_receipt`], this path deliberately does not consult the local command
    /// ID index.  The command ID is owner-generated and may be unknown to the caller when the
    /// mutation was applied but its response was lost.  The owner remains the authoritative,
    /// scoped source of the receipt.
    pub fn recover_receipt(
        ...
        self.require_owner("receipt_recovery")?;
        let scope = self.config.scope.clone();
        let idempotency_key = idempotency_key.to_owned();
        let receipt = self
            .client
            .call(|owner, auth| {
                owner.get_receipt_by_idempotency_key(auth, &scope, &idempotency_key)
            })
            .map_err(FacadeError::owner)
            .and_then(|receipt| self.project_receipt(receipt))?;
        self.cache_receipt(&receipt);
    ...
    /// Explicit alias for consumers that use the owner terminology in their client API.
    pub fn read_receipt_by_idempotency_key(
        &mut self,
        request: &FacadeRequest,
        idempotency_key: &str,
    ) -> FacadeResult<Receipt> {
        self.recover_receipt(request, idempotency_key)
    }
    ...
    pub fn receipts(&mut self, request: &FacadeRequest) -> FacadeResult<Vec<Receipt>> {
        ...
        for receipt in &receipts {
            evicted |= self.index.cache_receipt(receipt);
        }
        self.index.receipts_complete = !evicted;
        Ok(receipts)
    }
    ...
    pub fn create_draft(
        ...
        self.ensure_known_id(expected_active_revision_id, ReferenceKind::Revision)?;
    ...
    pub fn edit_draft(&mut self, request: &FacadeRequest, patch: Patch) -> FacadeResult<Draft> {
        ...
        // when the caller has edit authority but no metadata grant for an eligible-item refresh.
        self.cache_draft_references(&draft);
        self.cache_draft(draft.clone());
        Ok(draft)
    }
    ...
    pub fn preview(
        ...
        self.ensure_known_id(&preview.draft_id, ReferenceKind::Draft)?;
    ...
    fn cache_item(&mut self, item: ItemRef) {
        if self.index.cache_item(item) {
            self.index.items_complete = false;
        }
    }

    fn cache_revision_id(&mut self, revision_id: &str) {
        if bounded_set_insert(
            &mut self.index.revision_ids,
            &mut self.index.revision_id_order,
    ...
    fn cache_draft_references(&mut self, draft: &Draft) {
        for item in draft
            .selected_items
            .iter()
            .chain(draft.note_items.iter())
            .chain(draft.objective_item.iter())
        {
            self.cache_item(item.clone());
        }
    }

    fn cached_draft_references(&self) -> Vec<ItemRef> {
        self.index
            .drafts
            .values()
            .flat_map(|draft| {
    ...
    fn cache_preview(&mut self, preview: &Preview) {
        if self.index.cache_preview(preview) {
            self.index.previews_complete = false;
        }
    }

    fn cache_receipt(&mut self, receipt: &Receipt) {
        if self.index.cache_receipt(receipt) {
    ...
    fn validate_patch(&mut self, patch: &Patch) -> FacadeResult<()> {
        ...
        self.ensure_known_id(&patch.draft_id, ReferenceKind::Draft)?;
        self.ensure_current_draft_version(&patch.draft_id, patch.expected_draft_version)?;
        self.ensure_known_id(&patch.expected_active_revision_id, ReferenceKind::Revision)?;
        for operation in &patch.operations {
            ...
            match operation {
                ...
                Operation::RestoreConfiguration { source_revision_id } => {
                    self.ensure_known_id(source_revision_id, ReferenceKind::Revision)?;
                }
            }
        }
        Ok(())
    ...
    fn patch_requires_objective_authority(&mut self, patch: &Patch) -> FacadeResult<bool> {
        if patch
            .operations
            .iter()
        ...
        if !self.index.drafts.contains_key(&patch.draft_id) {
            self.revalidate_reference(&patch.draft_id, ReferenceKind::Draft)?;
        }
        if !self.index.revisions.contains_key(source_revision_id) {
            self.revalidate_reference(source_revision_id, ReferenceKind::Revision)?;
        }
    ...
    fn ensure_current_draft_version(
        ...
        if !self.index.drafts.contains_key(draft_id) {
            self.revalidate_reference(draft_id, ReferenceKind::Draft)?;
        }
    ...
    fn validate_command(&mut self, command: &Command, kind: &str) -> FacadeResult<()> {
        ...
        if let Some(revision_id) = command.expected_active_revision_id.as_deref() {
            self.ensure_known_id(revision_id, ReferenceKind::Revision)?;
        }
        for reference in [
        ...
        .flatten()
        {
            self.ensure_known_id(reference, ReferenceKind::Preview)?;
        }
                ...
                .call(|owner, auth| owner.eligible_items(auth, &scope))
            {
                Ok(items) => match self.project_items(items, false) {
                    Ok(items) => {
                        ...
                        let mut evicted = false;
                        for item in items {
                            evicted |= self.index.cache_item(item.item);
                        }
                        self.index.items_complete = !evicted;
                    }
    ...
    fn ensure_known_id(&mut self, id: &str, kind: ReferenceKind) -> FacadeResult<()> {
        if !valid_id(id) {
            return Err(FacadeError::invalid("invalid_reference"));
        }
        let (known, complete) = match kind {
            ReferenceKind::Revision => (
                self.index.revision_ids.contains(id),
                self.index.revisions_complete,
            ),
            ReferenceKind::Draft => (
                self.index.draft_ids.contains(id),
                self.index.drafts_complete,
            ),
            ReferenceKind::Preview => (
                self.index.preview_ids.contains(id),
                self.index.previews_complete,
            ),
            ReferenceKind::Receipt => (
                self.index.receipt_ids.contains(id),
        ...
        if known {
            ...
            self.revalidate_reference(id, kind)
        }
    }

    fn ensure_item(&mut self, item: &ItemRef) -> FacadeResult<()> {
            ...
            .get(&(item.item_id.clone(), item.version))
        {
            ...
            None => self.revalidate_item(item),
        }
    }

    fn revalidate_reference(&mut self, id: &str, kind: ReferenceKind) -> FacadeResult<()> {
        match kind {
            ReferenceKind::Revision => {
                if !self.owner_supports("revisions") {
                    return Err(FacadeError::owner(OwnerError::Unsupported));
                }
                let scope = self.config.scope.clone();
            ...
            ReferenceKind::Draft => {
                ...
                let draft = match self
                    ...
                    .call(|owner, auth| owner.get_draft(auth, &scope, &draft_id))
                {
                    Ok(draft) => self.project_draft(draft)?,
                    Err(OwnerError::NotFound) => {
                        return Err(FacadeError::denied("foreign_reference"));
                    }
                    Err(error) => return Err(FacadeError::owner(error)),
                };
            ...
            ReferenceKind::Preview => {
                if !self.owner_supports("previews") {
                    return Err(FacadeError::owner(OwnerError::Unsupported));
                }
                let scope = self.config.scope.clone();
                let preview_id = id.to_owned();
                let preview = match self
                    .client
                    .call(|owner, auth| owner.get_preview(auth, &scope, &preview_id))
                {
                    Ok(preview) => self.project_preview(preview)?,
                    Err(OwnerError::NotFound) => {
                        return Err(FacadeError::denied("foreign_reference"));
                    }
                    Err(error) => return Err(FacadeError::owner(error)),
            ...
            ReferenceKind::Receipt => {
                if !self.owner_supports("receipts") {
                    return Err(FacadeError::owner(OwnerError::Unsupported));
                }
                let scope = self.config.scope.clone();
                ...
                let receipt = match self
                    ...
                    .call(|owner, auth| owner.get_receipt(auth, &scope, &command_id))
                {
                    Ok(receipt) => self.project_receipt(receipt)?,
                    Err(OwnerError::NotFound) => {
                        return Err(FacadeError::denied("foreign_reference"));
                    }
                    Err(error) => return Err(FacadeError::owner(error)),
                };
                if receipt.command_id != id {
                    return Err(FacadeError::denied("foreign_reference"));
                }
                self.cache_receipt(&receipt);
                Ok(())
    ...
    fn revalidate_item(&mut self, item: &ItemRef) -> FacadeResult<()> {
        if !self.owner_supports("eligible_items") {
            return Err(FacadeError::owner(OwnerError::Unsupported));
        }
        let scope = self.config.scope.clone();
        let items = self
            ...
            .call(|owner, auth| owner.eligible_items(auth, &scope))
            .map_err(FacadeError::owner)
            .and_then(|items| self.project_items(items, false))?;
        let draft_references = self.cached_draft_references();
        let found = items
            .iter()
        ...
        for candidate in items {
            evicted |= self.index.cache_item(candidate.item);
        }
        for draft_reference in draft_references {
            evicted |= self.index.cache_item(draft_reference);
        }
        self.index.items_complete = !evicted;
        if let Some(found) = found {
            self.cache_item(found);
            Ok(())
        } else if self.index.items_complete {
            Err(FacadeError::denied("foreign_reference"))
        } else {
    ...
    fn owner_supports(&self, operation: &str) -> bool {
        self.owner_capabilities
            .as_ref()
            .is_some_and(|capabilities| {
                capabilities.enabled
                    && owner_supports_list(&capabilities.supported_operations, operation)
            })
    }
...
impl HarnessOwnerPort for crate::control::ControlPlane {
    ...
    fn get_receipt_by_idempotency_key(
        ...
        crate::control::ControlPlane::receipt_for_idempotency_key(self, idempotency_key)
            .map_err(OwnerError::from_control)
    }
