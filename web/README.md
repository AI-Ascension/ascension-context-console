...
`offline-bundle.json` manifest, which points at synthetic fixtures or the local demo endpoints.
...
- `js/app.js` - bootstrap and DOM wiring.
- `js/api.js` - same-origin fetch wrapper with a byte bound and `cache: "no-store"`.
...
- `js/render.js` - text-node rendering of the snapshot projection and comparison.
...
- The view is read-only; a comparison is a local display and cannot apply or submit context.
